...
public class CombinedDeveloperPredicate implements Predicate<Developer> {
    ...
    public CombinedDeveloperPredicate(Predicate<Developer> namePredicate, Predicate<Developer> rolePredicate,
                                      Predicate<Developer> addressPredicate, Predicate<Developer> dateJoinedPredicate
            , Predicate<Developer> emailPredicate, Predicate<Developer> phonePredicate,
                                      Predicate<Developer> projectPredicate, Predicate<Developer> salaryPredicate,
                                      Predicate<Developer> ratingPredicate, Predicate<Developer> githubIdPredicate) {
    ...
    public boolean test(Developer developer) {
        return namePredicate.test(developer) && rolePredicate.test(developer) && addressPredicate.test(developer)
                && dateJoinedPredicate.test(developer) && emailPredicate.test(developer)
                && phonePredicate.test(developer) && projectPredicate.test(developer) && salaryPredicate.test(developer)
                && ratingPredicate.test(developer) && githubIdPredicate.test(developer);
    }
}
